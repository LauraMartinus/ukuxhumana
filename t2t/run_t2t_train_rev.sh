...
mkdir -p "/tmp/t2t/${problem_name}_rev/data"
mkdir -p "/tmp/t2t/${problem_name}_rev/output"
mkdir -p "/tmp/t2t/${problem_name}_rev/tmp"
mkdir -p "/tmp/t2t/${problem_name}_rev/results"

# This just speeds up things since no need to download
cp $original_data_dir/$original_data_prefix.train.* /tmp/t2t/${problem_name}_rev/tmp
cp $original_data_dir/$original_data_prefix.dev.* /tmp/t2t/${problem_name}_rev/tmp
cp $original_data_dir/vocab*   /tmp/t2t/${problem_name}_rev/tmp


# Do training
t2t-trainer \
  --generate_data \
  --data_dir=/tmp/${problem_name}_rev/data \
  --output_dir=/tmp/${problem_name}_rev/output \
  --tmp_dir=/tmp/${problem_name}_rev/tmp \
  --problem=${problem_name}_rev \
  --model=transformer \
...
# Do decoding
t2t-decoder \
  --data_dir=/tmp/t2t/${problem_name}_rev/data \
  --problem=${problem_name}_rev \
  --model='transformer' \
  --hparams_set='transformer_base_single_gpu' \
  --output_dir=/tmp/t2t/${problem_name}_rev/output \
